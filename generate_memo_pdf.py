...
def build_pdf(filename="memo.pdf"):
    ...
    # ── HEADER ───────────────────────────────────────────────────────────────
    story.append(Paragraph("TECHNICAL MEMORANDUM: Real-Time Construction PPE Detection & Decision Layer", title_style))
    ...
    story.append(Paragraph(
        "We selected the Roboflow Universe <i>Construction Site Safety v2</i> dataset (CC BY 4.0), comprising 10,875 high-resolution "
        "construction scene images annotated across 12 safety equipment classes. Because the original dataset labeled equipment "
        "without annotating workers, we augmented the ontology to 13 classes by auto-labeling <b>Person</b> bounding boxes using "
        "a COCO-pretrained YOLO11 detector at confidence &ge; 0.50. This yields 11,018 worker instances across 62.4% of images, enabling "
        "our model to reason over both equipment presence and worker context in a single unified detector.",
        body_style
    ))
    ...
    metrics_data = [
        ...
        [Paragraph("<b>Validation (1,279 imgs)</b>", td_style), Paragraph("<b>79.90%</b>", td_center), Paragraph("<b>52.35%</b>", td_center), Paragraph("78.40%", td_center), Paragraph("79.49%", td_center), Paragraph("Peak fitness at Epoch 65 (Tesla T4, 71 epochs)", td_style)],
    ...
    story.append(Paragraph(
        "During baseline inspection, we detected that 4,990 annotation files (45.9% of the dataset) contained corrupted rows combining "
        ...
        "We engineered <font name='Courier'>fix_mixed_labels.py</font> to cleanse corrupted lines, recovering 21,529 valid bounding boxes across all splits. "
        "Additionally, discovering that Roboflow lacked a Person class led to our auto-labeling pipeline (<font name='Courier'>add_person_class.py</font>), "
    ...
    story.append(Paragraph(
        "To comply with architectural constraints forbidding agentic frameworks (no LangChain/CrewAI), we implemented a deterministic, "
        "pure-Python reasoning layer in <font name='Courier'>app/reasoning.py</font> across three explicit stages: "
